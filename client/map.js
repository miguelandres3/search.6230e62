  ...
  */
  ...
  var Searcher = function(prefs) {
    ...
    this.search = function(cb) {
      ...
      datarequest = { lat: this.prefs.position.lat, lng:this.prefs.position.lng, radius: this.prefs.radius,
      tags: this.prefs.tags, count: this.prefs.count}
      ...
      $.ajax({
        ...
        complete: function (data) {
          ...
          if(data.status==200)
          {
            ...
            cb(false,response.products);
          }else
          {
            var msg = 'response: ' + data.status + ' ' + data.statusText + ' '  + data.responseText;
